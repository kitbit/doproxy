  ...
  def get_inventory
    if File.exist?(@inventory_file) == false
      ...
      backend_inventory.each_line do |droplet_id|
        begin
          unless droplet_id.to_s.strip.empty?
            droplet = @client.droplets.find(id: URI.encode(droplet_id.strip))
            @droplets << droplet
            @backend_count += 1
          end
        rescue  
          raise "Inventory file contains a non-existent droplet id (#{droplet_id})!"
        end
      end
    end
  end
